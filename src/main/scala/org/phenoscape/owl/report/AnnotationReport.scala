...
object AnnotationReport {
    ...
    def render(obj: OWLObject): String = {
      ...
      val renderer = new ManchesterOWLSyntaxObjectRenderer(
        ...
        new AnnotationValueShortFormProvider(
          ...
          mutable.Map
            .empty[OWLAnnotationProperty, java.util.List[String]]
            .asJava,
          manager
        )
      )
  ...
  def main(args: Array[String]): Unit = {
    manager.loadOntologyFromOntologyDocument(
      new File(System.getProperty(ONTOLOGY_PROPERTY))
    );
    val builder = new SAXBuilder();
    val properties = mutable.Set[OWLObjectProperty]();
    val writer = new BufferedWriter(
      new FileWriter(System.getProperty(OUTPUT_PROPERTY))
    );
    writer.write(header);
    writer.newLine();
    for (arg <- args) {
        ...
        for (state <- states) {
          ...
          for (phenotype <- phenotypes) {
            ...
            if (phenotype.entity != null) {
              properties.asJava.addAll(
                phenotype.entity.getObjectPropertiesInSignature()
              );
            ...
            if (phenotype.quality != null) {
              properties.asJava.addAll(
                phenotype.quality.getObjectPropertiesInSignature()
              );
            ...
            if (phenotype.relatedEntity != null) {
              properties.asJava.addAll(
                phenotype.relatedEntity.getObjectPropertiesInSignature()
              );
  ...
  def getStates(
      character: Element,
      sets: Map[String, Iterable[Element]]
  ): Iterable[Element] = {
  ...
  def getPhenotypes(state: Element): Iterable[EQ] = {
    val phenotypes = state
      ...
      .iterator();
    return phenotypes.asScala
      .map(PhenoXMLUtil.translatePhenotypeCharacter(_))
      .toSeq;
  }
  ...
  def writeCharacter(character: Element, index: Int, writer: Writer): Unit = {
    ...
    writer.write("\t");
    writer.write(
      StringUtils.defaultString(character.getAttributeValue("label"))
    );
  }
