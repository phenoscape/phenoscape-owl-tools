...
object AnnotationReport {
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (arg <- args) {
      ...
      val stateSetsByID = stateSets.asScala
        .map(states => (states.getAttributeValue("id"), states.getChildren("state", nexmlNS).asScala))
        .toMap
  ...
  def getPhenotypes(state: Element): Iterable[EQ] = {
    ...
    phenotypes.asScala.map(PhenoXMLUtil.translatePhenotypeCharacter).toSeq
  }

  def getID(owlClass: OWLClassExpression): String =
    idRenderer.render(owlClass).replaceAll("\n", "").replaceAll("\\s+", " ")

  def getLabel(owlClass: OWLClassExpression): String =
    labelRenderer.render(owlClass).replaceAll("\n", "").replaceAll("\\s+", " ")
