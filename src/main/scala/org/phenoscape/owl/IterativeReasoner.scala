...
object IterativeReasoner extends OWLTask {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val mainOntology =
      mainManager.loadOntologyFromOntologyDocument(new File(args(0)));
  ...
  def computeInferences(
      mainOntology: OWLOntology,
      targetOntologies: Seq[OWLOntology]
  ): Unit = {
    val manager = mainOntology.getOWLOntologyManager();
    val aggregateTargetOntology = manager.createOntology();
    targetOntologies.foreach(target =>
      manager.addAxioms(aggregateTargetOntology, target.getAxioms())
    );
  ...
  def computeInferences(
    ...
    while (newAxiomCount > oldAxiomCount) {
      ...
      MaterializeInferences.materializeInferences(
        mainOntology,
        MaterializeInferences.createReasoner(mainOntology, "elk")
      );
      val hierarchy = ExtractNamedClassHierarchy.extractHierarchy(mainOntology);
      targetOntology
        .getOWLOntologyManager()
        .addAxioms(targetOntology, hierarchy.getAxioms());
      targetOntology
        .getOWLOntologyManager()
        .saveOntology(
          targetOntology,
          IRI.create(new File("/Users/jim/Desktop/target.owl"))
        );
      MaterializeInferences.materializeInferences(
        targetOntology,
        MaterializeInferences.createReasoner(targetOntology, "trowl")
      );
      mainOntology
        .getOWLOntologyManager()
        .addAxioms(mainOntology, targetOntology.getAxioms());
