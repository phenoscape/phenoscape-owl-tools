...
object NegationHierarchyAsserter {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val inputOntology: OWLOntology =
      manager.loadOntologyFromOntologyDocument(new File(args(0)))
    val axioms: Set[OWLAxiom] =
      inputOntology.getAxioms(Imports.INCLUDED).asScala.toSet
    val negationAxioms: Set[OWLAxiom] = assertNegationHierarchy(axioms)
    val negationOntology: OWLOntology =
      manager.createOntology(negationAxioms.asJava)
  ...
  def assertNegationHierarchy(axioms: Set[OWLAxiom]): Set[OWLAxiom] = {
    ...
    val classTuples = for {
      ...
      namedClasses: Set[OWLClass] = expr.collect {
        case owlClass: OWLClass => owlClass
      }
    ...
    val negatesPairs = for {
      ...
      namedNegationClasses = expr.collect {
        case owlClass: OWLClass => owlClass
      }
  ...
  def buildIndex[A, B](pairs: Iterable[(A, B)]): Map[A, Set[B]] =
    pairs.foldLeft(emptyIndex[A, B]) {
      case (index, (a, b)) => index.updated(a, index(a) + b)
    }
