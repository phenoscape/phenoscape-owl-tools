...
object HomologyTableToOWLVAHM extends App {
  ...
  val InHistoricalHomologyRelationshipWith = ObjectProperty(
    "http://purl.obolibrary.org/obo/RO_HOM0000007"
  )
  val SeriallyHomologousTo = ObjectProperty(
    "http://purl.obolibrary.org/obo/RO_HOM0000027"
  )
  val HistoricalHomologyMemberof = ObjectProperty(
    "http://example.org/historical_homology_member_of"
  )
  val HasHistoricalHomologyMember = ObjectProperty(
    "http://example.org/has_historical_homology_member"
  )
  val SerialHomologyMemberOf = ObjectProperty(
    "http://example.org/serial_homology_member_of"
  )
  val HasSerialHomologyMember = ObjectProperty(
    "http://example.org/has_serial_homology_member"
  )
  ...
  def convertFile(file: Source): OWLOntology = {
    ...
    manager.applyChange(
      new AddOntologyAnnotation(
        ...
        factory.getOWLAnnotation(
          description,
          factory.getOWLLiteral("Homology Assertions using the AVA model")
        )
      )
    )
    manager.addAxiom(
      ontology,
      HistoricalHomologyMemberof InverseOf HasHistoricalHomologyMember
    )
    manager.addAxiom(
      ontology,
      SerialHomologyMemberOf InverseOf HasSerialHomologyMember
    )
    manager.addAxiom(
      ontology,
    ...
    manager.addAxiom(
      ontology,
      SeriallyHomologousTo SubPropertyChain (SerialHomologyMemberOf o HasSerialHomologyMember)
    )
    ontology
  }
  ...
  def processEntry(line: String): Set[OWLAxiom] = {
    ...
    val uniquePrefix =
      s"http://purl.phenoscape.org/homology/annotation/$uniqueID"
    val relation = items(4).trim
    val (upProperty, downProperty, negated) = relation match {
      case "hom to" =>
        (HistoricalHomologyMemberof, HasHistoricalHomologyMember, false)
      case "ser hom to" =>
        (SerialHomologyMemberOf, HasSerialHomologyMember, false)
      case "not hom to" =>
        (HistoricalHomologyMemberof, HasHistoricalHomologyMember, true)
      case "not ser hom to" =>
        (SerialHomologyMemberOf, HasSerialHomologyMember, true)
    }
    val structure1Text = items(1).trim
    val structure1 =
      if (structure1Text.contains("^"))
        PostCompositionParser.parseExpression(structure1Text).get
      else Class(IRI.create(structure1Text))
    val taxon1 = Class(IRI.create(items(3).trim))
    val structure2Text = items(6).trim
    val structure2 =
      if (structure2Text.contains("^"))
        PostCompositionParser.parseExpression(structure2Text).get
