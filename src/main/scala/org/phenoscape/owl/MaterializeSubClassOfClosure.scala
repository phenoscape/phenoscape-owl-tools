...
object MaterializeSubClassOfClosure extends OWLTask {
  ...
  def main(args: Array[String]): Unit = {
    val targetFile = new File(
      System
        .getProperty("org.phenoscape.owl.MaterializeSubClassOfClosure.target")
    )
    val source = manager.loadOntologyFromOntologyDocument(new File(args(0)))
    println(
      manager.getOntologies().asScala.map(_.getAxiomCount()).reduce(_ + _)
    )
  ...
  def newMethod(source: OWLOntology): OWLOntology = {
    ...
    val axioms: Set[OWLAxiom] = createSubClassOfAxioms(
      factory.getOWLThing(),
      Set[OWLClass](),
      reasoner,
      doneClasses
  ...
  def createSubClassOfAxioms(
    ...
    if (!firstTime) {
      ...
      for (subclass <- directSubClasses.asScala) {
        axioms.asJava.addAll(
          newSuperClasses.map(factory.getOWLSubClassOfAxiom(subclass, _)).asJava
        )
      }
      val recursiveAxioms: Set[OWLAxiom] = directSubClasses.asScala
        .map(createSubClassOfAxioms(_, newSuperClasses, reasoner, doneClasses))
        .flatten
  ...
  def createSubClassOfAxioms(
    ...
    val axioms = for {
      superClass <- reasoner
        .getSuperClasses(owlClass, false)
        .getFlattened
        .asScala
