...
object ExpressionsUtil {
  ...
  def expressionForName(
      named: OWLClass
  ): Validation[String, OWLClassExpression] = {
    val parameters =
      URLEncodedUtils.parse(URI.create(named.getIRI.toString), "UTF-8")
    parameters.asScala
      .collectFirst({
  ...
  def instantiateClassAssertion(
    ...
    aClass match {
      ...
      case ObjectIntersectionOf(operands) => {
        operands.foreach(o =>
          axioms ++= instantiateClassAssertion(individual, o)
        )
      }
  ...
  def createEntityRenderer(
    ...
    entity => {
      ...
      val renderer =
        new ManchesterOWLSyntaxObjectRenderer(writer, shortFormProvider)
  ...
  def permute(expression: OWLClassExpression)(
      implicit reasoner: OWLReasoner
  ): Set[OWLClassExpression] = expression match {
    case namedClass: OWLClass =>
      (reasoner
        .getSuperClasses(namedClass, true)
        .getFlattened
        .asScala
        .toSet + namedClass).toSet[OWLClassExpression]
  ...
  )(implicit reasoner: OWLReasoner): Set[OWLObjectSomeValuesFrom] = {
  ...
  )(implicit reasoner: OWLReasoner): Set[OWLObjectIntersectionOf] = {
    val permutedOperands =
      expression.getOperands.asScala.toSet[OWLClassExpression].map(permute)
  ...
  def allCombinations[T](sets: Set[Set[T]]): Set[Set[T]] = {
    def combine[T](
        combinations: Set[Set[T]],
        itemsToCombine: Set[T]
    ): Set[Set[T]] =
      for {
