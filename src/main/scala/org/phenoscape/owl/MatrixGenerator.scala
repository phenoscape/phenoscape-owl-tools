...
object MatrixGenerator extends OWLTask {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val attributesSlim =
      manager.loadOntologyFromOntologyDocument(new File("character_slims.obo"));
    ...
    manager.applyChange(
      new AddImport(
        dataOntology,
        factory.getOWLImportsDeclaration(ro.getOntologyID.getOntologyIRI.get)
      )
    );
    manager.applyChange(
      new AddImport(
    ...
    val newAxioms =
      (for (entity <- entities.asScala; quality <- attributes.asScala)
  ...
  def annotateComposedEntityAndQuality(
    ...
    val entityAxiom = factory.getOWLAnnotationAssertionAxiom(
      entityTerm,
      subject,
      entity.getIRI()
    );
    val qualityAxiom = factory.getOWLAnnotationAssertionAxiom(
      qualityTerm,
      subject,
      quality.getIRI()
    );
  ...
  def compositionIRI(entity: OWLClass, quality: OWLClass): IRI = {
    IRI.create(
      "http://example.org/involves?entity=%s&quality=%s"
        .format(entity.getIRI(), quality.getIRI())
    );
  }
