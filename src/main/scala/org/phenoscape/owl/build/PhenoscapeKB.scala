...
object PhenoscapeKB extends KnowledgeBaseBuilder {
  ...
  def loadOntologiesAndCreateReasoner(): OWLReasoner = {
    ...
    val cl =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/cl.owl"), true)
    addTriples(cl, bigdata, graphURI)
    val homology = loadFromWeb(
      IRI.create("http://purl.org/phenoscape/demo/phenoscape_homology.owl"),
      true
    )
    addTriples(homology, bigdata, graphURI)
    val vtoToNCBI = loadFromWeb(
      IRI.create("http://purl.org/phenoscape/demo/vto_ncbi.owl"),
      true
    )
    addTriples(vtoToNCBI, bigdata, graphURI)
    val pato =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/pato.owl"), true)
    addTriples(pato, bigdata, graphURI)
    val bspo =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/bspo.owl"), true)
    addTriples(bspo, bigdata, graphURI)
    val go =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/go.owl"), true)
    addTriples(go, bigdata, graphURI)
    val taxrank = loadFromWeb(
      IRI.create("http://purl.obolibrary.org/obo/taxrank.owl"),
      true
    )
    addTriples(taxrank, bigdata, graphURI)
    val vto =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/vto.owl"), true)
    ...
    val zfa = loadFromWeb(
      IRI.create(
        "http://purl.obolibrary.org/obo/upheno/imports/zfa_import.owl"
      ),
      true
    )
    addTriples(zfa, bigdata, graphURI)
    val xao =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/xao.owl"), true)
    addTriples(xao, bigdata, graphURI)
    val hp =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/hp.owl"), true)
    addTriples(hp, bigdata, graphURI)
    val mp =
      loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/mp.owl"), true)
    addTriples(mp, bigdata, graphURI)
    val ro = loadFromWeb(
      IRI.create("http://purl.obolibrary.org/obo/ro.owl"),
      false
    ) //.axioms.filter(_.isAnnotationAxiom)
    addTriples(ro, bigdata, graphURI)
    val eco = loadFromWeb(
      IRI.create("http://purl.obolibrary.org/obo/eco/eco-base.owl"),
      false
    )
    ...
    val mgiToEMAPA = SourcedAxioms(
      loadNormalized((SOURCES / "mgi_anatomy.owl").toJava)
    )
    ...
    val emapaToUberon = loadFromWeb(
      IRI.create(
        "http://purl.obolibrary.org/obo/uberon/bridge/uberon-bridge-to-emapa.owl"
      ),
      true
    )
    ...
    val coreReasoner = reasoner(
      Set(uberon, cl, pato, bspo, phenoscapeVocab).flatMap(_.axioms)
    )
    ...
    val filesToConvert = (FileUtils
      .listFiles((NEXML / "completed-phenex-files").toJava, Array("xml"), true)
      .asScala ++
      FileUtils
        .listFiles(
          (NEXML / "fin_limb-incomplete-files").toJava,
          Array("xml"),
          true
        )
        .asScala ++
      FileUtils
        .listFiles(
          (NEXML / "Jackson_Dissertation_Files").toJava,
          Array("xml"),
          true
        )
        .asScala ++
      FileUtils
        .listFiles(
          (NEXML / "teleost-incomplete-files" / "Miniature_Monographs").toJava,
          Array("xml"),
          true
        )
        .asScala ++
      FileUtils
        .listFiles(
          (NEXML / "teleost-incomplete-files" / "Miniatures_Matrix_Files").toJava,
          Array("xml"),
          true
        )
        .asScala ++
      FileUtils
        .listFiles(
          (NEXML / "teleost-incomplete-files" / "Dillman_Supermatrix_Files").toJava,
          Array("xml"),
          true
        )
        .asScala ++
      FileUtils
        .listFiles((NEXML / "matrix-vs-monograph").toJava, Array("xml"), true)
    ...
    val zfinGenes = PropertyNormalizer.normalize(
      ZFINGeneticMarkersToOWL
        .convert((SOURCES / "zfin_genetic_markers.txt").toSource("ISO-8859-1"))
    )
    addTriples(zfinGenes, bigdata, graphURI)
    val zfinPreviousGeneNames = PropertyNormalizer.normalize(
      ZFINPreviousGeneNamesToOWL
        .convert((SOURCES / "zfin_aliases.txt").toSource("ISO-8859-1"))
    )
    ...
    val zfinExpressionData = PropertyNormalizer.normalize(
      ZFINExpressionToOWL.convert(
        (SOURCES / "zfin_wildtype_expression.txt").toSource("ISO-8859-1")
      )
    )
    addTriples(zfinExpressionData, bigdata, graphURI)
    val zfinPhenotypeData = PropertyNormalizer.normalize(
      ZFINPhenotypesToOWL
        .convert((SOURCES / "zfin_phenotypes.txt").toSource("ISO-8859-1"))
    )
    addTriples(zfinPhenotypeData, bigdata, graphURI)

    step("Converting MGI data")
    val mgiGenes = PropertyNormalizer.normalize(
      MGIGeneticMarkersToOWL
        .convert((SOURCES / "mgi_genes.txt").toSource("utf-8"))
    )
    addTriples(mgiGenes, bigdata, graphURI)
    val mgiExpressionData = PropertyNormalizer.normalize(
      MGIExpressionToOWL
        .convert((SOURCES / "mgi_expression_data.txt").toSource("utf-8"))
    )
    addTriples(mgiExpressionData, bigdata, graphURI)
    val mgiPhenotypeData = PropertyNormalizer.normalize(
      MGIPhenotypesToOWL
        .convert((SOURCES / "mgi_phenotypes.txt").toSource("utf-8"))
    )
    addTriples(mgiPhenotypeData, bigdata, graphURI)

    step("Converting Xenbase data")
    val xenbaseGenes = PropertyNormalizer.normalize(
      XenbaseGenesToOWL
        .convert((SOURCES / "xenbase_genes.txt").toSource("utf-8"))
    )
    ...
    val xenbasePhenotypeFiles = FileUtils.listFiles(
      (SOURCES / "xenbase-phenotypes").toJava,
      Array("txt"),
      true
    )
    ...
    val humanPhenotypeData = PropertyNormalizer.normalize(
      HumanPhenotypesToOWL
        .convert((SOURCES / "hp_phenotypes.txt").toSource(f"utf-8"))
    )
    ...
    val parts = anatomicalEntities.flatMap(
      NamedRestrictionGenerator.createRestriction(part_of, _)
    )
    ...
    val phenotypeOfPartOfs = anatomicalEntities.flatMap(
      NamedRestrictionGenerator.createRestriction(phenotype_of_part_of, _)
    )
    val absences =
    ...
    val attributeQualities = attributes.axioms.flatMap(
      _.getClassesInSignature.asScala
    ) + HasNumberOf
    val subsumers = for {
      entity <- anatomicalEntities
      (partsTerm, entityPartsAxioms) = SimilarityTemplates.partsOfEntity(entity)
      (developsFromTerm, developsFromAxioms) = SimilarityTemplates
        .developsFromEntity(entity)
    ...
    println(
      "tbox class count: " + allTBox
        .flatMap(_.getClassesInSignature.asScala)
        .size
    )
    println(
      "tbox logical axiom count: " + allTBox.filter(_.isLogicalAxiom).size
    )
    ...
    val coreTboxOntology =
      manager.createOntology(coreTBoxWithoutDisjoints.asJava)
    val coreTboxReasoner = reasoner(coreTboxOntology)
    MaterializeInferences.materializeInferences(
      coreTboxOntology,
      coreTboxReasoner
    )
    ...
    val hierarchyAxioms = NegationHierarchyAsserter.assertNegationHierarchy(
      tBoxWithoutDisjoints ++ inferredAxioms.getAxioms().asScala
    )
    ...
    implicit val negationReasoner = reasoner(
      tBoxWithoutDisjoints ++ inferredAxioms.getAxioms().asScala
    )
    ...
    val tboxOut = OWLManager
      ...
      .createOntology(
        (tBoxWithoutDisjoints ++ inferredAxioms.getAxioms().asScala).asJava
      )
    ...
    val vtoOnt =
      OWLManager.createOWLOntologyManager().createOntology(vto.axioms.asJava)
//    bigdata.add(EvolutionaryProfiles.computePhenotypeProfiles(TaxonNode(CHORDATA), vtoOnt, bigdata).asJava, graphURI)
  ...
  val presencesQuery = construct(t('taxon, Vocab.has_presence_of, 'entity)) from "http://kb.phenoscape.org/" where (bgp(
    ...
    t(
      ...
      IRI.create("http://purl.obolibrary.org/obo/uberon.owl")
    )
  ),
  ...
  val absencesQuery = construct(t('taxon, Vocab.has_absence_of, 'entity)) from "http://kb.phenoscape.org/" where (bgp(
    t(
      'taxon,
      Vocab.exhibits_state / Vocab.describes_phenotype / (rdfsSubClassOf *) / ABSENCE_OF,
      'entity
    ),
    t(
      'entity,
  ...
  val presencesOutput = new BufferedOutputStream(
    new FileOutputStream(presencesFile)
  )
  bigdata
    .prepareGraphQuery(QueryLanguage.SPARQL, presencesQuery.toString)
    .evaluate(new TurtleWriter(presencesOutput))
  presencesOutput.close()

  step("Exporting absence assertions")
  val absencesFile = (KB / "absences.ttl").toJava
  val absencesOutput = new BufferedOutputStream(
    new FileOutputStream(absencesFile)
  )
  bigdata
    .prepareGraphQuery(QueryLanguage.SPARQL, absencesQuery.toString)
    .evaluate(new TurtleWriter(absencesOutput))
  ...
  val triplesQuery = bigdata.prepareGraphQuery(
  ...
  val triplesOutput = new BufferedOutputStream(
    new FileOutputStream((KB / "kb.ttl").toJava)
  )
...
WHERE {
  ...
  val profilesOutput = new BufferedOutputStream(
    new FileOutputStream((KB / "profiles.ttl").toJava)
  )
