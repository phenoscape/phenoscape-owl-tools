...
object CharaparserEvaluationDB extends KnowledgeBaseBuilder {
  ...
  val ext = loadNormalized(new File(cwd + "/staging/sources/phenoscape-ext-simple.owl"))
  val termRequests = loadNormalized(new File(cwd + "/staging/sources/term_requests_ALL.owl"))
