...
object PhenexToOWL {
  ...
  def taxonForOTU(otu: Element): Option[OWLNamedIndividual] =
    getResourceMetaValues(otu, "taxonID", dwcNS).headOption
      .filterNot(_.toString.startsWith(ProvisionalPrefix))
      .map(Individual)
  ...
  def translateState(
    ...
    val phenotypeAxioms =
      if (possiblePhenotypeAxioms.exists(
          _.getSignature.asScala.exists(_.getIRI.toString.startsWith(ProvisionalPrefix)))) Set.empty
