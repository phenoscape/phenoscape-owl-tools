...
object PhenexToOWL extends OWLTask {
  ...
  val dcCreator =
    factory.getOWLAnnotationProperty(DublinCoreVocabulary.CREATOR.getIRI)
  val dcBibliographicCitation = factory.getOWLAnnotationProperty(
    IRI.create("http://rs.tdwg.org/dwc/terms/bibliographicCitation")
  )
  val phenoscapeComplement =
    IRI.create("http://purl.obolibrary.org/obo/PHENOSCAPE_complement_of")
  val manager = OWLManager.createOWLOntologyManager
  type LabelRenderer = OWLObject => String

  def convert(
      file: File,
      vocabulary: OWLOntology = manager.createOntology()
  ): OWLOntology = {
    val labelRenderer =
      ExpressionsUtil.createEntityRenderer(factory.getRDFSLabel, vocabulary)
    ...
    val (charactersAxioms, characterToOWLMap) =
      translateCharacters(nexml, matrix, labelRenderer)
    val matrixAxioms =
      translateMatrixRows(nexml, matrix, taxonOTUToOWLMap, characterToOWLMap)
  ...
  def translateMatrix(
      nexml: Element,
      fileName: String
  ): (OWLNamedIndividual, Set[OWLAxiom]) = {
    val sourceAxioms = for {
      sourceMeta <- getResourceMetasForProperty(nexml, "source", dcTermsNS)
      matrix <- getLiteralMetaValues(sourceMeta, "identifier", dcTermsNS).map(
        Individual
      )
      citation <- getLiteralMetaValues(
        sourceMeta,
  ...
  def translateMatrixRow(
    ...
    cells
      .flatMap(translateMatrixCell(_, otuID, owlOTU, characterToOWLMap))
      .toSet
  }
  ...
  def translateMatrixCell(
    ...
    val states =
      if (characterToOWLMap.contains(stateID)) Set(stateID)
        ...
        getElementByID(stateID, cell)
          .getChildren("member", nexmlNS)
          .asScala
          .map(_.getAttributeValue("state"))
          .toSet
    val stateAxioms = for {
      singleState <- states
      ...
      val taxonStateAxiomOpt =
        for (owlTaxon <- taxonForOTU(getElementByID(otuID, cell)))
          yield owlTaxon Fact (exhibits_state, owlState)
  ...
  def translateOTUs(
    ...
    val translations = nexml
      .getChild("otus", nexmlNS)
      .getChildren("otu", nexmlNS)
      .asScala
      .map(translateOTU(_, matrix))
  ...
  def translateOTU(
    ...
    (
      axioms ++ otuToValidTaxonOption ++ otuComments ++ specimensAxioms,
      otuID -> owlOTU
    )
  }
  ...
  def translateSpecimen(
      specimenElement: Element,
      owlOTU: OWLNamedIndividual
  ): Set[OWLAxiom] = {
    ...
    val catalogNumAxiom = for {
      value <- getLiteralMetaValues(specimenElement, "catalogNumber", dwcNS)
    } yield factory.getOWLDataPropertyAssertionAxiom(
      catalogNumber,
      owlSpecimen,
      value
    )
  ...
  def translateCharacters(
    ...
    val characterElements = nexml
      .getChild("characters", nexmlNS)
      .getChild("format", nexmlNS)
      .getChildren("char", nexmlNS)
      .asScala
    val charactersWithIndex = characterElements.zipWithIndex
    val translations = charactersWithIndex.map {
      case (characterElement, index) =>
        translateCharacter(characterElement, index, matrix, labelRenderer)
    }
  ...
  def translateCharacter(
    ...
    val characterLabel =
      optString(character.getAttributeValue("label")).getOrElse("")
    val axioms = Set(
    ...
    val (statesAxioms, stateToOWLMap) =
      translateStates(statesBlock, owlCharacter, characterLabel, labelRenderer)
    (
      axioms ++ characterComments ++ statesAxioms,
      stateToOWLMap + (characterID -> owlCharacter)
    )
  }
  ...
  def translateStates(
    ...
    val translations = stateElements.map(
      translateState(_, owlCharacter, characterLabel, labelRenderer)
    )
  ...
  def translateState(
    ...
    val stateSymbol =
      optString(state.getAttributeValue("symbol")).getOrElse("<?>")
  ...
  def translatePhenotypes(
      state: Element,
      owlState: OWLNamedIndividual,
      labelRenderer: LabelRenderer
  ): Set[OWLAxiom] = {
    val phenotypeElements = state
      .getDescendants(new ElementFilter("phenotype_character", phenoNS))
      .iterator
      .asScala
      .toSet
    val (phenotypeOptions, phenotypeAxiomSets) = phenotypeElements
      .map(phenotypeElement =>
        translatePhenotypeSemantics(phenotypeElement, labelRenderer)
      )
      .unzip
    val linksToPhenotypes =
      phenotypeOptions.flatten.map(owlState Annotation (describes_phenotype, _))
  ...
  def translatePhenotypeSemantics(
    ...
    val (entityAndLabelOption, entityAxioms) = optionWithSet(for {
      ...
      val (entity, entityLabel, axioms) =
        namedClassFromTyperef(bearerType, labelRenderer)
      annotations += factory.getOWLAnnotation(entity_term, entity.getIRI)
      (
        (entity, entityLabel),
        axioms ++ AbsenceClassGenerator.generateAllAbsenceAxiomsForEntity(
          entity
        )
      )
    })
    val entityLabel = entityAndLabelOption.map(_._2).getOrElse("")
    val qualityElementOption = Option(
      phenotypeElement.getChild("quality", phenoNS)
    )
    val (qualityAndLabelOption, qualityAxioms) = optionWithSet(for {
      ...
      val (quality, qualityLabel, axioms) =
        namedClassFromTyperef(qualityType, labelRenderer)
    ...
    val (relatedEntityAndLabelOption, relatedEntityAxioms) = optionWithSet(for {
      qualityElement <- qualityElementOption
      relatedEntityElement <- Option(
        qualityElement.getChild("related_entity", phenoNS)
      )
      relatedEntityType <- Option(
        relatedEntityElement.getChild("typeref", phenoNS)
      )
    } yield {
      val (relatedEntity, relatedEntityLabel, axioms) =
        namedClassFromTyperef(relatedEntityType, labelRenderer)
      annotations += factory
        .getOWLAnnotation(related_entity_term, relatedEntity.getIRI)
      (
    ...
    val relatedEntityLabel =
      relatedEntityAndLabelOption.map(_._2).map(re => s" $re").getOrElse("")
    val eqPhenotypeOption = (
      entityAndLabelOption.map(_._1),
      qualityAndLabelOption.map(_._1),
      relatedEntityAndLabelOption.map(_._1)
    ) match {
      ...
      case (Some(entity), Some(quality), None) =>
        Option(has_part some (quality and (inheres_in some entity)))
      case (Some(entity), Some(quality), Some(relatedEntity)) =>
        Option(
          has_part some (quality and (inheres_in some entity) and (towards some relatedEntity))
        )
    ...
    annotations += factory.getOWLAnnotation(
      RDFSLabel,
      factory.getOWLLiteral(phenotypeLabel)
    )
    val annotationAxioms = phenotypeClass
      .map(term =>
        annotations.map(factory.getOWLAnnotationAssertionAxiom(term.getIRI, _))
      )
      .toSet
      .flatten
    (
      phenotypeClass,
      entityAxioms ++ qualityAxioms ++ relatedEntityAxioms ++ phenotypeAxioms ++ annotationAxioms
    )
  }
  ...
  def classFromTyperef(typeref: Element): OWLClassExpression = {
    ...
    if (qualifiers.isEmpty) genus
      ...
      factory.getOWLObjectIntersectionOf(
        (qualifiers.map(restrictionFromQualifier).toSet + genus).asJava
      )
  }

  def namedClassFromTyperef(
      typeref: Element,
      labelRenderer: LabelRenderer
  ): (OWLClass, String, Set[OWLAxiom]) = {
    classFromTyperef(typeref) match {
      case named: OWLClass => (named, labelRenderer(named), Set.empty)
      case expression =>
        val (named, axioms) =
          ExpressionUtil.nameForSubClassWithAxioms(expression)
        val expressionLabel = labelRenderer(expression)
        (
          named,
          expressionLabel,
          axioms + (named Annotation (rdfsLabel, expressionLabel))
        )
    }
  }
  ...
  def restrictionFromQualifier(qualifier: Element): OWLClassExpression = {
    ...
    if (propertyIRI == phenoscapeComplement)
      factory.getOWLObjectComplementOf(filler)
    else {
  ...
  def getLiteralMetaValues(
    ...
    val values = for {
      ...
      if meta.getAttributeValue("property") == (prefixForNamespace(
        meta,
        namespace
      ) + property)
      value <- optString(meta.getValue) ++ optString(
  ...
  def getResourceMetasForProperty(
      element: Element,
      property: String,
    ...
    for {
      meta <- element.getChildren("meta", nexmlNS).asScala
      if meta.getAttributeValue("rel") == (prefixForNamespace(
        meta,
        propertyNamespace
      ) + property)
    } yield meta
  ...
  def optString(text: String): Option[String] =
    Option(StringUtils.stripToNull(text))

  def getElementByID(id: String, elementInDoc: Element): Element = {
    val elements = elementInDoc.getDocument.getRootElement
