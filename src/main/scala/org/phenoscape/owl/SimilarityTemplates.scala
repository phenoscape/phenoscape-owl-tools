...
object SimilarityTemplates {
  ...
  def partsOfEntity(entity: OWLClass): (OWLClass, Set[OWLAxiom]) =
    ExpressionUtil.nameForExpressionWithAxioms(
      phenotype_of some (entity or (part_of some entity))
    )

  def developsFromEntity(entity: OWLClass): (OWLClass, Set[OWLAxiom]) =
    ExpressionUtil.nameForExpressionWithAxioms(
      phenotype_of some (entity or (DEVELOPS_FROM some entity))
    )

  def entityWithQuality(
      entity: OWLClass,
      quality: OWLClass
  ): (OWLClass, Set[OWLAxiom]) =
    ExpressionUtil.nameForExpressionWithAxioms(
      (has_part some quality) and (phenotype_of some entity)
    )

  def partsOfEntityWithQuality(
      entity: OWLClass,
      quality: OWLClass
  ): (OWLClass, Set[OWLAxiom]) =
    ExpressionUtil.nameForExpressionWithAxioms(
