...
class SubsumerGenerator(ont: OWLOntology, reasonerFactory: OWLReasonerFactory) {
  ...
  val expressionMakers: Set[EQMaker] = Set(
    new EQMaker(Class(Vocab.ANATOMICAL_ENTITY), Class(Vocab.QUALITY))
  )
    ...
    def next: scala.collection.Set[EQMaker] =
      for {
        ...
        entity <- EntitySearcher
          .getSubClasses(rootEntity, ont)
          .asScala
          .toSet + rootEntity
        if !entity.isAnonymous
        quality <- EntitySearcher
          .getSubClasses(rootQuality, ont)
          .asScala
          .toSet + rootQuality
  ...
  private def generateSubsumers(
    ...
    if (nextGeneration.isEmpty) allNewAxioms
      ...
      generateSubsumers(nextGeneration.seq, allNewAxioms, tested ++ testMakers)
  }
