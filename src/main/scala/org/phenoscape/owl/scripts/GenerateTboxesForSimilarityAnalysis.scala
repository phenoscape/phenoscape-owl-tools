...
object GenerateTboxesForSimilarityAnalysis extends App {
  ...
  val profilesTbox =
    ParseProfileSemantics.tboxWithSemanticsForProfiles(profiles)
  val phenoscapeVocab = loadFromWebWithImports(
    "http://purl.org/phenoscape/vocab.owl"
  )
  val uberon = loadFromWebWithImports(
  ...
  val attributes = loadFromWebWithImports(
  ...
  val (entityAttributePhenotypes, entityAttributePhenotypeAxioms) =
    flattenAxioms((for {
  ...
  val (entityPartAttributePhenotypes, entityPartAttributePhenotypeAxioms) =
    flattenAxioms((for {
      attribute <- attributeQualities
      entity <- anatomicalEntities
    } yield {
  ...
  val entitiesOnt = manager.createOntology(
    (mainTbox ++ entityPhenotypeAxioms ++ entityPartsPhenotypeAxioms).asJava
  )
  ...
  val entitiesAttributesOnt = manager.createOntology(
    (mainTbox ++ entityAttributePhenotypeAxioms ++ entityPartAttributePhenotypeAxioms).asJava
  )
  ...
  val entitiesAndQualitiesReasoner =
    new ElkReasonerFactory().createReasoner(entitiesAndQualitiesOnt)
  MaterializeInferences.materializeInferences(
    entitiesAndQualitiesOnt,
    entitiesAndQualitiesReasoner
  )
  entitiesAndQualitiesReasoner.dispose()

  val entitiesAttributesReasoner =
    new ElkReasonerFactory().createReasoner(entitiesAttributesOnt)
  MaterializeInferences.materializeInferences(
    entitiesAttributesOnt,
    entitiesAttributesReasoner
  )
  entitiesAttributesReasoner.dispose()

  val reducedEntitiesOnt = OntologyUtil.reduceOntologyToHierarchy(entitiesOnt)
  val reducedEntitiesAndQualitiesOnt =
    OntologyUtil.reduceOntologyToHierarchy(entitiesAndQualitiesOnt)
  val reducedEntitiesAttributesOnt =
    OntologyUtil.reduceOntologyToHierarchy(entitiesAttributesOnt)

  manager.saveOntology(reducedEntitiesOnt, IRI.create(new File("entities.owl")))
  manager.saveOntology(
    reducedEntitiesAndQualitiesOnt,
    IRI.create(new File("entitiesAndQualities.owl"))
  )
  manager.saveOntology(
    reducedEntitiesAttributesOnt,
    IRI.create(new File("entitiesAndAttributes.owl"))
  )

  val entityWriter = new OutputStreamWriter(
    new FileOutputStream("entity_phenotypes.txt"),
    "UTF-8"
  )
  entityPhenotypes.foreach(p => entityWriter.write(s"${p.getIRI.toString}\n"))
  entityWriter.close()

  val entityPartsWriter = new OutputStreamWriter(
    new FileOutputStream("entity_parts_phenotypes.txt"),
    "UTF-8"
  )
  entityPartsPhenotypes.foreach(p =>
    entityPartsWriter.write(s"${p.getIRI.toString}\n")
  )
  entityPartsWriter.close()

  val qualityWriter = new OutputStreamWriter(
    new FileOutputStream("quality_phenotypes.txt"),
    "UTF-8"
  )
  qualityPhenotypes.foreach(p => qualityWriter.write(s"${p.getIRI.toString}\n"))
  qualityWriter.close()

  val entitiesAttributesWriter = new OutputStreamWriter(
    new FileOutputStream("entity_attribute_phenotypes.txt"),
    "UTF-8"
  )
  entityAttributePhenotypes.foreach(p =>
    entitiesAttributesWriter.write(s"${p.getIRI.toString}\n")
  )
  entitiesAttributesWriter.close()

  val entityPartsAttributesWriter = new OutputStreamWriter(
    new FileOutputStream("entity_part_attribute_phenotypes.txt"),
    "UTF-8"
  )
  entityPartAttributePhenotypes.foreach(p =>
    entityPartsAttributesWriter.write(s"${p.getIRI.toString}\n")
  )
...
object GenerateTboxesForSimilarityAnalysisUtil {
  ...
  def loadFromWebWithImports(iri: String): Set[OWLAxiom] = {
    ...
    val importsAxioms =
      (ont.getImportsClosure.asScala - ont).flatMap(_.getAxioms().asScala)
    manager.addAxioms(ont, importsAxioms.asJava)
    PropertyNormalizer.normalize(ont)
    ont.getAxioms().asScala.toSet
  }

  def flattenAxioms[A, B](in: (A, Set[Set[B]])): (A, Set[B]) =
    (in._1, in._2.flatten)
