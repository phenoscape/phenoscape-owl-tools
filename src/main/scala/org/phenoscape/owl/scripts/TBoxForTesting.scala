...
object TBoxForTesting extends KnowledgeBaseBuilder {
  ...
  val zfaToUberon =
    loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/uberon/bridge/uberon-ext-bridge-to-zfa.owl"), false)
  val xaoToUberon =
    loadFromWeb(IRI.create("http://purl.obolibrary.org/obo/uberon/bridge/uberon-bridge-to-xao.owl"), false)
  ...
  val anatomicalEntities =
    coreReasoner.getSubClasses(Class(Vocab.ANATOMICAL_ENTITY), false).getFlattened.asScala.filterNot(_.isOWLNothing)
  ...
  val zfinExpressionData = PropertyNormalizer.normalize(
    ZFINExpressionToOWL
      .convert(Source.fromFile(new File(cwd + "/staging/sources/zfin_wildtype_expression.txt"), "ISO-8859-1"))
  )
  val zfinPhenotypeData = PropertyNormalizer.normalize(
    ZFINPhenotypesToOWL.convert(Source.fromFile(new File(cwd + "/staging/sources/zfin_phenotypes.txt"), "ISO-8859-1"))
  )

  step("Converting MGI data")
  val mgiPhenotypeData = PropertyNormalizer.normalize(
    MGIPhenotypesToOWL.convert(Source.fromFile(new File(cwd + "/staging/sources/mgi_phenotypes.txt"), "utf-8"))
  )
  ...
  val xenbasePhenotypeFiles =
    FileUtils.listFiles(new File(cwd + "/staging/sources/xenbase-phenotypes"), Array("txt"), true)
  ...
  val humanPhenotypeData = PropertyNormalizer.normalize(
    HumanPhenotypesToOWL.convert(Source.fromFile(new File(cwd + "/staging/sources/hp_phenotypes.txt"), "utf-8"))
  )
