...
object EQCreator {
  ...
  def main(args: Array[String]): Unit = {
    ...
    manager.addAxiom(
      eqs,
      factory.getOWLTransitiveObjectPropertyAxiom(has_part)
    );
    ...
    manager.addAxiom(
      eqs,
      factory.getOWLInverseObjectPropertiesAxiom(has_part, part_of)
    );
    val uberon =
      manager.loadOntologyFromOntologyDocument(new File("uberon.owl"));
    val pato = manager.loadOntologyFromOntologyDocument(new File("pato.owl"));
    manager.applyChange(
      new AddImport(
        eqs,
        factory
          .getOWLImportsDeclaration(uberon.getOntologyID.getOntologyIRI.get)
      )
    );
    manager.applyChange(
      new AddImport(
        eqs,
        factory.getOWLImportsDeclaration(pato.getOntologyID.getOntologyIRI.get)
      )
    );
    val anatomicalEntity = Class(
      IRI.create("http://purl.obolibrary.org/obo/UBERON_0001062")
    );
    val qualityRoot = Class(
      IRI.create("http://purl.obolibrary.org/obo/PATO_0000001")
    );
    ...
    val anatomicalEntities =
      uberonReasoner.getSubClasses(anatomicalEntity, false).getFlattened();
    uberonReasoner.dispose();
    val qualities =
      patoReasoner.getSubClasses(qualityRoot, false).getFlattened();
    ...
    val outputStream = new GZIPOutputStream(
      new BufferedOutputStream(new FileOutputStream(new File(args(0) + ".gz")))
    );
  ...
  def createEQ(entity: OWLClass, quality: OWLClass): OWLAxiom = {
    val eqClass = Class(
      IRI.create(entity.getIRI().toString() + "+" + quality.getIRI().toString())
    );
