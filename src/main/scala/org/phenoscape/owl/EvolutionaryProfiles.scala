...
object EvolutionaryProfiles {
  ...
  val MayHaveStateValue =
    ResourceFactory.createProperty(may_have_state_value.getIRI.toString)
  val ExhibitsState =
    ResourceFactory.createProperty(exhibits_state.getIRI.toString)
  val DescribesPhenotype =
    ResourceFactory.createProperty(describes_phenotype.getIRI.toString)
  ...
  def computePhenotypeProfiles(
    ...
    val (associations, profiles) =
      postorder(rootTaxon, model, index(observedAssociations), Map.empty)
    profilesToRDF(profiles, model)
  }

  def report(
      profiles: Map[TaxonNode, Map[Character, Set[State]]],
      taxonomy: OWLOntology
  ): Unit = {
    for {
      ...
      val taxonLabel =
        ExpressionsUtil.labelFor(taxon, taxonomy).getOrElse("unlabeled")
  ...
  def index(
      associations: Set[StateAssociation]
  ): Map[TaxonNode, Map[Character, Set[State]]] =
  ...
  def profilesToRDF(
      profiles: StateAssociations,
      model: Model
  ): Set[Statement] = {
    val statePhenotypes: Map[State, Set[Phenotype]] = queryStatePhenotypes(
      model
    )
    (for {
  ...
  implicit def taxonToOWLClass(taxon: TaxonNode): OWLClass =
    factory.getOWLClass(taxon.iri)
  ...
  def toSequential(
    ...
    associations
      .map({ case (taxon, states) => taxon -> states.seq.toMap })
      .seq
      .toMap
  ...
  def postorder(
    ...
    val children = (for {
      s <- model
        .listStatements(
          null,
          ResourceFactory.createProperty(rdfsSubClassOf.toString),
          node.asJenaNode
        )
        .asScala
        .toList
    ...
    val nodeStates =
      startingAssociations.getOrElse(node, Map.empty[Character, Set[State]])
    if (children.isEmpty) {
      (Map(node -> nodeStates), Map.empty)
    } else {
      val (subtreeAssociationsGroups, subtreeProfilesGroups) = children.par
        .map(postorder(_, model, startingAssociations, startingProfiles))
        .unzip
      val subtreeAssociations = subtreeAssociationsGroups.flatten.toMap
      val subtreeProfiles = subtreeProfilesGroups.flatten.toMap
      val charactersWithAssociations = subtreeAssociations.values
        .flatMap(_.keys)
        .toSet ++ nodeStates.keys
      val currentNodeAssociationsAndProfile = for {
        ...
        val childrenStateSets =
          children.map(subtreeAssociations(_).getOrElse(character, Set.empty))
      ...
      val (currentNodeAssociations, rawProfile) =
        currentNodeAssociationsAndProfile.unzip
      val profileWithoutEmptyCharacters = rawProfile.filter {
        case (character, states) => states.nonEmpty
      }
      (
  ...
  def queryStatePhenotypes(model: Model): Map[State, Set[Phenotype]] = {
    ...
    results.asScala
      .map { bindings =>
        (
          State(
            IRI.create(bindings.getResource("state").getURI),
            bindings.getLiteral("state_label").getLexicalForm
          ),
  ...
  def apply(result: QuerySolution): StateAssociation =
    StateAssociation(
      ...
      State(
        ...
        result.getLiteral("state_label").getLexicalForm
      )
    )
